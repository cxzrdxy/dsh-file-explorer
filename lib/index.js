...
async function updateFile(path, content, expectedSize, expectedMtime) {
	...
	try {
		...
		if (error.code === "ENOENT") throw codeError("文件不存在", "ENOENT");
		throw error;
	}
	if (st.isDirectory()) throw codeError("不能编辑目录", "EISDIR");
	if (expectedSize !== undefined && st.size !== expectedSize) throw codeError("文件已被其他进程修改（大小变化）", "CONFLICT");
	if (expectedMtime !== undefined) {
		const currentMtime = st.mtime.toISOString();
		if (currentMtime !== expectedMtime) throw codeError("文件已被其他进程修改（时间变化）", "CONFLICT");
	}
	if (content.length > MAX_TEXT_BYTES) throw codeError("文件内容过大", "ETOOBIG");
...
async function handle(req, res) {
	...
	try {
		...
		if (action === "update") {
			json(res, 200, {
				ok: true,
				value: await updateFile(typeof payload.path === "string" ? cleanPath(payload.path) : "", typeof payload.content === "string" ? payload.content : "", typeof payload.expectedSize === "number" ? payload.expectedSize : undefined, typeof payload.expectedMtime === "string" ? payload.expectedMtime : undefined)
			});
			return;
		}
